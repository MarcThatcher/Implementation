...
-- trans ; first parameter is flag - if True are translating a rule -> attributes need to be flagged "int"
trans :: Bool -> Term -> VarName -> Net -> LUT -> Net
-- Variables
trans isRule (Var v) root (agents, wires) _ =
    (agents, wires ++ [(v, root)])

-- Empty net vanishes
trans isRule Empty _ (agents, wires) _ =
    (agents, wires)

-- Constructors
trans isRule (Constr constr attr args) root (agents, wires) lut =
  let 
      numIns = length args

      -- generate auxiliary ports from the root, adding attribute (singular) with "int" if a rule
      aP        = take numIns $ tail $ iterate fresh root
      attrPorts = if isRule && not (null attr) then ("int "++attr) else attr
      auxPorts  = if null attrPorts then aP else attrPorts:aP
      ...
      (agents', wires') =
        foldl
          (\(as, ws) (arg, aroot, auxPort) ->
             let (as', ws') = trans isRule arg aroot (as, ws) lut
...
-- functions
trans isRule (Func fName attr args) root net lut =
  let numOuts = funcNumOuts fName lut
      ...
      newAgent = if null attrPorts then (fName, pp, outPorts ++ inPortsExPP)
                 else (fName, pp, outPorts ++ inPortsExPP ++ [attrPorts])
        ...
        foldr
          (\(arg, port) (as, ws) ->
             let freshP       = fresh port
                 (as', ws')  = trans isRule arg freshP net lut
...
-- let
trans isRule (Let t1 vars t2) root net lut =
  let
      -- 1. Translate t1
      (agents1, wires1) = trans isRule t1 root net lut
      ...
      --   else trans isRule t2 (root++"L"++(fresh root)) (agents1',wires1') lut
      (agents2,wires2) = 
        if null agents1' then (agents1',wires1')
        else 
          let (agents3,wires3) = trans isRule t2 (root++"L"++(fresh root)) ([],[]) lut
...
-- ||
trans isRule (Par t1 t2) root (agents,wires) lut =
  let (agents1, wires1) = trans isRule t1 root (agents,wires) lut
      ...
      (agents2, wires2) = trans isRule t2 newRoot (agents1,wires1) lut
  in (agents2, wires2)
...
renameOutputs Empty agent vars lut = 
  undefined
renameOutputs (Constr _ attr _) (name,out,ins) vars lut =
  let [newOut] = vars   -- should only be 1
  in (name,newOut,ins)
renameOutputs (Func fName attr inputs) (name,pp,auxs) vars lut = 
...
countOuts Empty _        = 0
countOuts (Var _) _       = 1
countOuts (Constr _ _ _) _  = 1
countOuts (Func f _ _) lut   = funcNumOuts f lut
...
transRule (Rule t1 t2) lut =
    let lhs = cleanNet $ trans True t1 "r" ([], []) lut
        rhs = case t2 of
                Par t1' t2' -> 
                    let flatRHS = flatPar (Par t1' t2')
                        ...
                        combineNet (agentsAcc, wiresAcc) (term, port) =
                            -- set trans isRule flag to False as RHS does not have int in front of attributes
                            let (a', w') = trans False term port ([], []) lut
                            in (agentsAcc ++ a', wiresAcc ++ w')
                        rhsNet = foldl combineNet ([], []) (zip flatRHS outsList)
                    in rhsNet
                Let _ _ _ -> cleanUpLetOutputs lhs (cleanNet $ trans False t2 "r" ([], []) lut) lut
                _         -> cleanNet $ trans False t2 "r" ([], []) lut
...
tr lut term =
  case parseTerm term of
    Left err -> Left err
    Right t  -> Right (cleanNet $ trans False t "r" ([], []) lut) 
