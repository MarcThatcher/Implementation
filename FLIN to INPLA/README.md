...
2. Extended - attributes (integers only) and syntactic sugar for lists.
...
Use single letter variable names or even better those at the end of the alphabet - x,xs,y etc - as internally FLIN uses rr,rs,st and so on.
...
Syntactic sugar for lists: [x,y,z] gets desugared to Cons(x,Cons(y,Cons(z,Nil))).
Therefore [] becomes Nil.
So results in INPLA will be Cons not [...].
